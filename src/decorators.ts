...
export const addClass = (selector: string, ...clasNames: string[]) => (constructor: any, methodName: string, descriptor: PropertyDescriptor): any => {
    if(Reflect.getMetadata("design:returntype", constructor, methodName) === HTMLElement) {
        ...
        descriptor.value = function wrapper(...args) {
            ...
            content.querySelectorAll(selector).forEach(elem => clasNames.forEach(c => elem.classList.add(c)));
            return content;
        }
    }
}